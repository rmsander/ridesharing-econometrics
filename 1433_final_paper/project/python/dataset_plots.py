...
years_arr = df["Year"][3:]
cities = list(set(df["Urban Area"][3:]))
print(len(cities))
...
both = df["UberXlyft"][3:]
uber_plus_lyft = np.add(uber,lyft)
...
for year, pop, auto, free, art, gen_time, comm_time, gas, diesel, excess_gas, \
    ann_delay, travel_index, cost, ub, ly, bo, ump in zip(years_arr,
                                                          population, \
                                                          auto_commuters,
                                                          free_traffic,
                                                          arterial_traffic,
                                                          general_time_value, \
                                                          commercial_time_value,
                                                          gasoline_cost,
                                                          diesel_cost, \
                                                          excess_fuel_per_commuter, \
                                                          annual_hrs_of_delay,
                                                          travel_time_index,
                                                          cost_per_autocommuter,
                                                          uber, lyft, \
                                                          both, unemployment):

    # Now we can start appending values
